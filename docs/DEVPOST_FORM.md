...
- **MongoDB (partner MCP):** the agent queries Atlas through the **official `mongodb-mcp-server`** registered as an ADK `MCPToolset` — and uses **Atlas Vector Search as its memory** (14 consumer-protection precedents + 11 recovery playbooks across US/UK/EU, embedded with gemini-embedding-001, native cosine vector indexes).
...
### What's next
Google OAuth verification for the Gmail connector; one-click pre-filled claim forms; a cross-session recovered-to-date ledger; multi-currency rule packs (UK/EU/India).
